...
def input_stock():
    try:
        global stock
        stock = input("Qual o ticker da companhia? ")[:4].upper()

        if google != True:
            googleAPI()

        print('LOAD', stock, 'data from sheet')
        list = ws_bovespa_uberlista.get_all_values()
        search = stock
        for r, row in enumerate(list):
            if search == row[8][:4]:  # first 4 characters in company ticker column
                company = row
                break
        print('...done ')
    except Exception as e:
...
def getSheetListOfReports(c):
    try:
        ...
        if newsheet_id == '':
            name = {'name': str(company[8][:5] + ' / ' + company[9][:5] + ' / ' + company[10][:5] + ' - ' + company[5][0][:4].replace('Nenh','NONE') + ' - ' + company[1] + ' - ' + company[2] + ' - ' + company[0])}
...
# ee = singleone_project() # all in one
