...
def user_defined_variables():
    try:
        ...
        template_sheet = '1R5YOiJOWGhjgvEQdtst_PqrPMeGLV1qe9imjA4lk6mg'
...
def loadSheets():
    try:
        ...
        restart(e, __name__)
def loadCompanySheets(company):
    try:
        global sheet_index
        global sheet_reports

        company_sheet = gsheet.open_by_key(company [col ['PLANILHA']].replace(sheet_url, ''))
        sheet_index = company_sheet.worksheet('index')
        sheet_reports = company_sheet.worksheet('reports')
    except Exception as e:
        restart(e, __name__)
...
def createSheet(company):
    try:
        ...
        company_id = newsheet.get('id')
        company [col ['PLANILHA']] = sheet_url + company_id
        permission_user = {
            'type': 'user',
            'role': 'writer',
        ...
        # permission = gdrive.permissions().create(fileId=company_id, body=permission_user, fields="id").execute()
        # permission_id = permission.get('id')

        print('...create sheet', company [col ['PREGÃO']], company_id)
        return company
    except Exception as e:
        restart(e, __name__)
def setCompanyMainPage(company):
    try:
        ...
        if google != True:
            google = googleAPI()
        loadCompanySheets(company)

        # update company sheet
        sheet_index.resize(6, 6)
        sheet_index.update_acell('F4', company [col ['CMV']])
        sheet_index.update_acell('A1', company [col ['PREGÃO']])
        sheet_index.update_acell('F1', company [col ['EMPRESA']])
        sheet_index.update_acell('D4', company [col ['MERCADO']])
        # sheet_index.update_acell('A1', company[col['LINK']])
        # sheet_index.update_acell('A1', company[col['PLANILHA']])
        sheet_index.update_acell('F2', company [col ['DATA']])
        sheet_index.update_acell('A3', company [col ['SITE']])
        sheet_index.update_acell('F3', company [col ['CNPJ']])
        sheet_index.update_acell('E4', company [col ['TICKER']])
        sheet_index.update_acell('A4', company [col ['SETOR']])
        sheet_index.update_acell('B4', company [col ['SUBSETOR']])
        sheet_index.update_acell('C4', company [col ['SEGMENTO']])
        sheet_index.update_acell('A2', company [col ['ATIVIDADE']])
...
def getSheetCompanyReports(company):
    try:
        ...
        sheet_report_list = sheet_index.get_all_values()
        # sheet_data = sheet_reports.get_all_values()
        sheet_report_list = [line for line in sheet_report_list if 'http' in line [0]]
...
def companySheet(company):
    try:
        ...
        global company_id
        if company [col ['PLANILHA']]:
            company_id = company [col ['PLANILHA']].replace(sheet_url, '')
            action = 'found'
        else:
            company = getCompanyMainPage(company)
            company_id = createSheet(company) [col ['PLANILHA']].replace(sheet_url, '')
        ...
        print('...done', action, company [col ['PREGÃO']], sheet_url + company_id)
        return company
    except Exception as e:
        restart(e, __name__)
    ...
    try:
        ...
        for cell in cell_list:
            try:
                ...
                pass
        sheet_index.update_cells(cell_list)
